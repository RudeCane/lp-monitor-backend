...
async function fetchGeckoDex(cfg) {
...
function normalizePool(p, cfg) {
  ...
  if (nm.includes('0.01%')) fee = 0.0001;
  else if (nm.includes('0.05%')) fee = 0.0005;
  else if (nm.includes('0.3%'))  fee = 0.003;
  else if (nm.includes('1%'))    fee = 0.01;
  ...
  return {
    id: p.id, addr,
    name: a.name || 'Unknown',
    dexId: cfg.id, dexName: cfg.short,
    chain: cfg.chain, network: cfg.network,
    liq, vol24: v24, vol6h: v6, vol1h: v1,
    buys:   t24.buys  || 0,
    sells:  t24.sells || 0,
...
module.exports = { getCache, setCache, cors, DEX_CONFIGS, fetchGeckoDex };
